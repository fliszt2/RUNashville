  ...
  render() {
    ...
    return (
      <div>
        MANAGE POSTS
        {allPosts.map((post) => (
          <SocialPost key={post.date + post.post} propic={post.propic} name={post.name} date={post.date} location={post.location} image={post.image} run={post.run} post={post.post} />
        ))}
      </div>
    );
  }
